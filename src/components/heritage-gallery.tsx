...
export function HeritageGallery() {
  ...
  return <>
    ...
    <div className="heritage-gallery-grid">{visible.map((photo) => <button className="heritage-photo-card" data-reveal type="button" key={photo.src} onClick={() => setSelected(photo)} aria-label={`Perbesar foto: ${photo.title}, ${photo.place}`}><span className="heritage-photo-image"><Image src={photo.src} alt={`${photo.title} di ${photo.place}`} fill sizes="(max-width: 650px) 100vw, (max-width: 950px) 50vw, 33vw" /></span><span className="heritage-photo-info"><span>{photo.place}</span><strong>{photo.title}</strong><b aria-hidden="true"><ArrowIcon /></b></span></button>)}</div>
    {selected && <div className="heritage-lightbox" role="dialog" aria-modal="true" aria-label={`${selected.title}, ${selected.place}`} onClick={() => setSelected(null)}><button className="heritage-lightbox-close" type="button" onClick={() => setSelected(null)} aria-label="Tutup foto">×</button><div className="heritage-lightbox-content" onClick={(event) => event.stopPropagation()}><div className="heritage-lightbox-image"><Image src={selected.src} alt={`${selected.title} di ${selected.place}`} fill sizes="95vw" /></div><p><strong>{selected.title}</strong><span>{selected.place}</span></p></div></div>}
  </>;
}
