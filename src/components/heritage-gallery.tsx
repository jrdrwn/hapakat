...
export function HeritageGallery() {
  ...
  return <>
    ...
    <div className="heritage-gallery-grid">{visible.map((photo) => <button className="heritage-photo-card" type="button" key={photo.src} onClick={() => setSelected(photo)} aria-label={`Perbesar foto: ${photo.title}, ${photo.place}`}><span className="heritage-photo-image"><Image src={photo.src} alt={`${photo.title} di ${photo.place}`} fill sizes="(max-width: 650px) 100vw, (max-width: 950px) 50vw, 33vw" /></span><span className="heritage-photo-info"><span>{photo.place}</span><strong>{photo.title}</strong><b aria-hidden="true">↗</b></span></button>)}</div>
