...
export function LanguageExplorer() {
  ...
  return <div className="language-explorer">
    ...
    <div className="language-results">{visible.length ? visible.map((story) => <Link href={`/galeri/${story.slug}`} className="language-result" key={story.slug}><span className="language-result-copy"><small>{story.language}</small><strong>{story.title}</strong><span>{story.subtitle}</span></span><b aria-hidden="true">↗</b></Link>) : <p className="language-empty">Tidak ada ungkapan yang cocok. Coba kata atau bahasa lain.</p>}</div>
